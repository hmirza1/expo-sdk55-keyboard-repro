    ...
    <>
      ...
      <Stack.Toolbar placement="right">
        <Stack.Toolbar.Menu icon="ellipsis.circle">
          <Stack.Toolbar.MenuAction
            icon="pencil"
            onPress={() => handleHeaderAction("Header menu: Edit")}
          >
            Edit
          </Stack.Toolbar.MenuAction>
          <Stack.Toolbar.MenuAction
            icon="square.and.arrow.up"
            onPress={() => handleHeaderAction("Header menu: Share")}
          >
            Share
          </Stack.Toolbar.MenuAction>
          <Stack.Toolbar.MenuAction
            icon="doc.on.doc"
            onPress={() => handleHeaderAction("Header menu: Duplicate")}
          >
            Duplicate
          </Stack.Toolbar.MenuAction>
          <Stack.Toolbar.MenuAction
            icon="bookmark"
            onPress={() => handleHeaderAction("Header menu: Bookmark")}
          >
            Bookmark
          </Stack.Toolbar.MenuAction>
          <Stack.Toolbar.MenuAction
            icon="trash"
            destructive
            onPress={() => handleHeaderAction("Header menu: Delete")}
          >
            Delete
          </Stack.Toolbar.MenuAction>
        </Stack.Toolbar.Menu>
      ...
      >
        ...
        <Text className="mt-2 text-sm text-[#5F5F5F]">
          Minimal repro: TextInput + native Stack.Toolbar menu + inline Zeego +
          ActionSheetIOS (iOS only).
        </Text>
        ...
        <View className="mt-4 flex-col items-start gap-3">
          <Text className="text-xs text-[#5F5F5F]">
            Native Stack.Toolbar menu (tap ... in header):
          </Text>
          <Text className="text-xs text-[#999999]">
            Uses Stack.Toolbar.Menu with native UIMenu — test if this also
            triggers the bug
          </Text>
        </View>
        ...
        <TouchableOpacity
          ...
          className="mt-6 h-11 items-center justify-center rounded-xl border border-[#D9D9D9] bg-white"
        >
